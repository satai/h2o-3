...
def glm_alpha_array_lambda_null():
    ...
    for l in range(0,len(r['lambdas'])):
        ...
        m = glm(family='multinomial',alpha=[r['alphas'][l]],Lambda=[r['lambdas'][l]], max_iterations=10)
        ...
        assert abs(devm - devn) < 1e-2
        pyunit_utils.assertEqualRegPaths(regKeys, r, l, mr, tol=1e-2)
        if (l == best_submodel_index): # check training metrics, should equal for best submodel index
            pyunit_utils.assertEqualModelMetrics(m._model_json["output"]["training_metrics"],
                                                 ...
                                                 keySet=["MSE","null_deviance", "logloss", "RMSE", "r2"],tol=1e-2)
...
def assertCoefEqual(regCoeff, coeff, coeffClassSet, tol=5e-1):
    for key in regCoeff:
        temp = key.split('_')
        classInd = int(temp[1])
